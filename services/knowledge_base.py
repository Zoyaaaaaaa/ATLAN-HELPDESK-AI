...
class EnhancedAtlanKnowledgeBase:
    ...
    def _setup_collection(self):
        """Setup Qdrant collection with proper error handling and indexes"""
        try:
            collections = self.qdrant.get_collections()
            collection_names = [col.name for col in collections.collections]
    ...
    def _create_payload_indexes(self):
        ...
        for field_name, field_type in indexes_to_create:
            try:
                ...
                print(f"   ⚠️ Index creation for '{field_name}' failed: {str(e)}")
                # This might fail if index already exists, which is okay
                continue

    def _ensure_payload_indexes(self):
        """Ensure payload indexes exist for existing collection"""
        print("🔍 Checking payload indexes...")
        
        try:
            # Try to get collection info to see existing indexes
            collection_info = self.qdrant.get_collection(self.collection_name)
            
            # Create indexes (this will fail silently if they already exist)
            self._create_payload_indexes()
            
        except Exception as e:
            print(f"   ⚠️ Error checking/creating indexes: {str(e)}")
    ...
    def search_knowledge_base(self, query: str, limit: int = 5, category_filter: Optional[str] = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Build filter conditions only if category_filter is provided
            filter_conditions = None

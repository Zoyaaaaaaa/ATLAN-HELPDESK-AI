...
def get_sample_tickets() -> List[Dict[str, Any]]:
    """
    Get sample tickets data from JSON structure
...
def save_tickets_to_json(tickets: List[Dict[str, Any]], filename: str = "tickets.json") -> bool:
    """
...
def load_tickets_from_json(filename: str = "tickets.json") -> List[Dict[str, Any]]:
    """
    Load tickets from JSON file, fallback to sample data if file doesn't exist
    """
    try:
        ...
        print(f"Error loading tickets: {e}, using sample data")
        return get_sample_tickets()
